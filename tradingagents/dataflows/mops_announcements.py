...
def _fetch_detail(client: _Client, ann: _Announcement, code: str, board: str) -> None:
    ...
    try:
        ...
        if published != ann.published:
            ...
            return
        # A misaligned reference could return another announcement with the same
        # timestamp. MOPS pads detail subjects with spaces, so compare them
        # normalized (NFKC, whitespace removed) but otherwise exactly.
        detail_subject = _text(_DETAIL_API, row[mapping["subject"]], "主旨")
        if norm_header(detail_subject) != norm_header(ann.subject):
            ann.detail_status = "detail unavailable: detail subject does not match the list"
            return
        ann.detail = {
...
def get_material_announcements(
    ...
    Lists announcements published within ``look_back_days`` calendar dates
    and fetches full detail for at most the newest ``detail_limit`` (capped at
    5). A historical run covers the ``look_back_days`` dates before
    ``curr_date`` and excludes ``curr_date`` itself; a live run covers the
    ``look_back_days`` dates ending today, including what was published up to
    now. A detail is used only when its timestamp and subject match the list.
    ...
    if live:
        # Today counts as one of the look_back_days dates, so a live run sees the
        # same number of calendar dates as a historical one.
        window_start = today - timedelta(days=look_back_days - 1)
        window_end = today
    else:
