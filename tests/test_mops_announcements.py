...
# --- hardening: live window size and detail subject ------------------------------------------

class LiveWindowAndDetailSubjectTests(_Base):
    def test_live_window_covers_exactly_look_back_days_calendar_dates(self):
        # Live on 2026-09-15 with 30 days: 2026-08-17 .. 2026-09-15 is 30 dates,
        # the same span a historical run gets (as_of - 30 .. as_of - 1).
        self.NOW = "2026-09-15 12:00:00"
        self.serve(_Ann("2026-08-16 10:00:00", "OUTSIDE-31st-day"),
                   _Ann("2026-08-17 10:00:00", "INSIDE-30th-day"))
        out = self.run_tool("2330.TW", "2026-09-15", look_back_days=30)
        self.assertIn("INSIDE-30th-day", out)
        self.assertNotIn("OUTSIDE-31st-day", out)
        self.assertIn("published from 2026-08-17", out)
        self.assertEqual(self.mops.list_calls()[0]["firstDay"], "17")

    def test_historical_window_still_covers_look_back_days_dates(self):
        self.serve(_Ann("2026-08-15 10:00:00", "OUTSIDE"), _Ann("2026-08-16 10:00:00", "INSIDE"))
        out = self.run_tool("2330.TW", "2026-09-15", look_back_days=30)
        self.assertIn("INSIDE", out)
        self.assertNotIn("OUTSIDE", out)

    def test_detail_with_same_timestamp_but_different_subject_is_rejected(self):
        ann = _Ann("2026-09-11 18:03:01", "董事會決議", text="不該被採用的說明")
        self.serve(ann)
        other = ann.detail_result()
        other["data"][0][6] = "另一筆公告的主旨"        # 主旨 column in the detail titles
        self.mops.detail_override[(ann.enter, ann.serial)] = _ok(other)
        out = self.run_tool("2330.TW", "2026-09-15")
        self.assertIn("detail unavailable: detail subject does not match the list", out)
        self.assertIn("| 董事會決議 |", out)                # list row kept
        self.assertNotIn("不該被採用的說明", out)
        self.assertNotIn("另一筆公告的主旨", out)

    def test_detail_subject_padding_and_line_breaks_still_match(self):
        # Live MOPS pads detail subjects with trailing spaces. The comparison also
        # tolerates line-break style and full-width spacing differences, which a
        # plain strip() would not, so formatting-only drift never rejects a real
        # detail.
        ann = _Ann("2026-09-11 18:46:58", "本公司財務報告業經\r\n董事會決議通過", text="採用的說明")
        self.serve(ann)
        padded = ann.detail_result()
        padded["data"][0][6] = "本公司財務報告業經\n董事會決議通過　" + " " * 120
        self.mops.detail_override[(ann.enter, ann.serial)] = _ok(padded)
        out = self.run_tool("2330.TW", "2026-09-15")
        self.assertIn("| fetched |", out)
        self.assertIn("採用的說明", out)
