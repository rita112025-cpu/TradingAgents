...
from tradingagents.dataflows import interface, mops_announcements as ma
from tradingagents.dataflows.errors import NoMarketDataError
from tradingagents.dataflows.mops import MopsUnavailableError
...
def _taipei(iso: str) -> datetime:
    return datetime.fromisoformat(iso).replace(tzinfo=ma._TAIPEI)
...
class HttpTests(unittest.TestCase):
    def _fails(self, msg_part, **kw):
        with mock.patch.object(ma.requests, "post", **kw), \
    ...
    def test_oversized_body(self):
        resp = _FakeResponse(body=b"x" * (ma._MAX_BODY_BYTES * 2))
        self._fails("exceeded", return_value=resp)
        self.assertLessEqual(resp.bytes_served, ma._MAX_BODY_BYTES + ma._CHUNK_BYTES)
        self._fails("exceeded", return_value=_FakeResponse(
            body=b"{}", headers={"Content-Length": str(ma._MAX_BODY_BYTES + 1)}))

    def test_request_is_json_post_bounded_and_verified(self):
        resp = _FakeResponse(body=b'{"code": 200, "result": {}}')
        with mock.patch.object(ma.requests, "post", return_value=resp) as post:
        ...
        self.assertEqual(kwargs["timeout"], ma._TIMEOUT)
        self.assertTrue(kwargs["stream"])
        self.assertEqual(kwargs["headers"]["User-Agent"], ma._UA)
