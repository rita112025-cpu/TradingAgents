...
# --- hardening: expected unit and duplicate company rows ------------------------------------

class UnitAndDuplicateRowTests(_Base):
    def _fails(self, part, look_back_months=1):
        with self.assertRaises(mops.MopsUnavailableError) as ctx:
            mops.get_monthly_revenue("2330.TW", _TODAY, look_back_months=look_back_months)
        self.assertIn(part, str(ctx.exception))

    def test_unexpected_unit_fails_closed(self):
        self.server.add("sii", 2026, 8, _table([_TSMC], unit="元"))
        self._fails("unit is '元', expected '千元'")

    def test_any_unit_label_on_the_page_must_be_the_expected_one(self):
        page = _table([_TSMC]).replace(
            "</body>".encode("cp950"),
            "<table><tr><th>單位：百萬元</th></tr></table></body>".encode("cp950"))
        self.server.add("sii", 2026, 8, page)
        self._fails("unit is '百萬元', expected '千元'")

    def test_mixed_units_across_months_cannot_silently_pass(self):
        # The first month is in 千元, the second is not: the report must not be
        # labelled 千元 on the strength of the first table alone.
        self.server.add("sii", 2026, 8, _table([_TSMC]))
        self.server.add("sii", 2026, 7, _table([_TSMC], unit="元"))
        self._fails("unit is '元', expected '千元'", look_back_months=2)

    def test_expected_unit_with_padding_is_accepted(self):
        self.server.add("sii", 2026, 8, _table([_TSMC], unit="  千元"))
        out = mops.get_monthly_revenue("2330.TW", _TODAY, look_back_months=1)
        self.assertIn("Unit: 千元 as stated by MOPS", out)

    def test_duplicate_company_row_fails_closed(self):
        self.server.add("sii", 2026, 8, _table([_TSMC, _NEG, _TSMC]))
        self._fails("company 2330 appears in more than one row")
